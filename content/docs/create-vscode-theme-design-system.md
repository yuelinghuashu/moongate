...
v2.1.0 引入的**海拔系统**将设计哲学从代码语法延伸到了界面本身。通过定义四层明度阶梯，让界面元素拥有真实的物理层次：

| 海拔层级 | 用途 | 深色模式特征 | 浅色模式特征 |
|----------|------|--------------|--------------|
| `surfaceGround` | 底层背景 | 最暗 | 最亮 |
| `surfaceRaised` | 侧边栏、活动栏 | 略微抬升 | 略微下沉 |
| `surfaceFloating` | 面板、悬浮卡片 | 明显漂浮 | 明显凸起 |
| `surfaceTooltip` | 提示框、弹窗 | 最高层 | 最深阴影 |
